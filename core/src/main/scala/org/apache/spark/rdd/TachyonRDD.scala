...
import org.apache.spark.{InterruptibleIterator, Logging, Partition, SerializableWritable, SparkContext, TaskContext, SparkEnv}
...
class TachyonRDD[T: ClassManifest](
  ...
  override def compute(theSplit: Partition, context: TaskContext) = new Iterator[T] {
    ...
    var tachyonBuf = file.readByteBuffer(0)
    if (tachyonBuf == null) {
      file.recache()
      tachyonBuf = file.readByteBuffer(0)
    }
    val buf = tachyonBuf.DATA
    val tachyonSerializer = SparkEnv.get.closureSerializer.newInstance()
